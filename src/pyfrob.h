...
// frame first).
typedef std::vector<Thread> (*get_threads_t)(pid_t, PyAddresses, bool, bool, Unwinder*);

// Frobber to get python stack stuff; this encapsulates all of the Python
// interpreter logic.
class PyFrob {
 public:
  PyFrob(pid_t pid, bool enable_threads, bool enable_cstack, Unwinder *unwinder)
      : pid_(pid),
        enable_threads_(enable_threads),
        enable_cstack_(enable_cstack),
        unwinder_(unwinder){}
